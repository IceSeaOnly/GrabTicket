...
function initScreen() {//初始化屏幕
    jsonGet(rootUrl + "/user/textShow", "", function (data) {
        $("#textShowContent").text(data);
    })
    timer = setInterval(function () {
        jsonGet(rootUrl + "/user/danMuList", "", function (data) {
            $("#wrap").append(createScreenPraise(data.headimg, data.message, data.vote, data.state, data.id));
        })
    }, 4000);
}


function jsonGet(url, params, func) {
